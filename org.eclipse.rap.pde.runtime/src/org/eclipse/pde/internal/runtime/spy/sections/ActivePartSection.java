...
public class ActivePartSection implements ISpySection {
	...
	public void build(ScrolledForm form, SpyFormToolkit toolkit, ExecutionEvent event) {
		...
		section.setText(NLS.bind(PDERuntimeMessages.get().SpyDialog_activePart_title, part.getSite().getRegisteredName()));
		...
		buffer.append(toolkit.createClassSection(text, NLS.bind(PDERuntimeMessages.get().SpyDialog_activePart_desc, partType), new Class[] {part.getClass()}));
		if (part instanceof PageBookView) {
			PageBookView outline = (PageBookView) part;
			IPage currentPage = outline.getCurrentPage();
			if (currentPage != null) {
				buffer.append(toolkit.createClassSection(text, PDERuntimeMessages.get().SpyDialog_activePageBook_title, new Class[] {currentPage.getClass()}));
			}
		}
