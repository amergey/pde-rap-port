...
public class LocalRegistryBackend implements IRegistryEventListener,
		BundleListener, ServiceListener, RegistryBackend {
	 ...
	 * 
	 * @see
	 * org.eclipse.pde.internal.runtime.registry.model.local.RegistryBackend
	 * #connect()
	 */
	...
	protected static boolean isRegisteredService(
			org.osgi.framework.Bundle bundle, ServiceReference ref) {
	...
	protected static boolean isServiceInUse(org.osgi.framework.Bundle bundle,
		...
		return (usingBundles != null && Arrays.asList(usingBundles).contains(
				bundle));
	}
	...
	public MultiStatus diagnose(long id) {
		PlatformAdmin plaformAdmin = PDERuntimePlugin.getDefault()
				.getPlatformAdmin();
		State state = plaformAdmin.getState(false);

		BundleDescription desc = state.getBundle(id);

		PlatformAdmin platformAdmin = PDERuntimePlugin.getDefault()
				.getPlatformAdmin();
		...
		for (int i = 0; i < resolverErrors.length; i++) {
			if ((resolverErrors[i].getType() & (ResolverError.MISSING_FRAGMENT_HOST
				...
				continue;
			IStatus status = new Status(IStatus.WARNING, PDERuntimePlugin.ID,
	...
	public void initializeServices(IProgressMonitor monitor) {
		...
		try {
			references = PDERuntimePlugin.getDefault().getBundleContext()
	...
	private Bundle createBundleAdapter(org.osgi.framework.Bundle bundle) {
		...
		adapter.setVersion((String) bundle.getHeaders().get(
		...
		String fragmentHost = (String) bundle.getHeaders().get(
				Constants.FRAGMENT_HOST);
		if (fragmentHost != null) {
			ManifestElement[] header;
			try {
				header = ManifestElement.parseHeader(Constants.FRAGMENT_HOST,
						fragmentHost);
		...
		BundlePrerequisite[] imports = (BundlePrerequisite[]) getManifestHeaderArray(
				bundle, Constants.REQUIRE_BUNDLE);
		if (imports != null)
			adapter.setImports(imports);

		BundleLibrary[] libraries = (BundleLibrary[]) getManifestHeaderArray(
				bundle, Constants.BUNDLE_CLASSPATH);
		if (libraries != null)
			adapter.setLibraries(libraries);

		BundlePrerequisite[] importPackages = (BundlePrerequisite[]) getManifestHeaderArray(
				bundle, Constants.IMPORT_PACKAGE);
		if (importPackages != null)
			adapter.setImportedPackages(importPackages);

		BundlePrerequisite[] exportPackages = (BundlePrerequisite[]) getManifestHeaderArray(
				bundle, Constants.EXPORT_PACKAGE);
	...
	private Extension createExtensionAdapter(IExtension extension) {
		...
		adapter.setExtensionPointUniqueIdentifier(extension
	...
	private ConfigurationElement createConfigurationElement(
	...
	private static Long getBundleId(String name) {
		BundleDescription descr = PDERuntimePlugin.getDefault()
				.getPlatformAdmin().getState(false).getBundle(name, null);
		return descr == null ? null : new Long(descr.getBundleId());
	}

	private ExtensionPoint createExtensionPointAdapter(
		...
		adapter.setContributor(getBundleId(extensionPoint.getContributor()
				.getName()));

		Extension[] extensions = createExtensionAdapters(extensionPoint
				.getExtensions());
		adapter.getExtensions().addAll(Arrays.asList(extensions));
		return adapter;
	}

	private ServiceRegistration createServiceReferenceAdapter(
			ServiceReference ref) {
		ServiceRegistration service = new ServiceRegistration();
		service.setId(((Long) ref
		...
		if (propertyKeys != null) {
			...
			for (int p = 0; p < propertyKeys.length; p++) {
				...
				properties[p] = new Property(key,
						ServiceRegistration.toString(value));
			}
		}
	...
	private Object[] getManifestHeaderArray(org.osgi.framework.Bundle bundle,
			String headerKey) {
		String libraries = (String) bundle.getHeaders().get(headerKey);
		try {
			ManifestElement[] elements = ManifestElement.parseHeader(headerKey,
					libraries);
			if (elements == null)
				return null;
			if (headerKey.equals(Constants.BUNDLE_CLASSPATH)) {
				...
				for (int i = 0; i < elements.length; i++) {
					...
					if (headerKey.equals(Constants.REQUIRE_BUNDLE)) {
						prereq.setVersion(element
								.getAttribute(Constants.BUNDLE_VERSION_ATTRIBUTE));
						String visibility = element
								.getDirective(Constants.VISIBILITY_DIRECTIVE);
						prereq.setExported(Constants.VISIBILITY_REEXPORT
								.equals(visibility));
					} else {
						prereq.setVersion(element
	...
	private Attribute[] createConfigurationElementAttributes(
			IConfigurationElement config) {
	...
	private static String createName(IConfigurationElement config) {
		...
		if (label == null && config.getAttribute("id") != null) { //$NON-NLS-1$
			...
			label = labelSplit.length == 0 ? null
	...
	private ExtensionPoint[] createExtensionPointAdapters(
	...
	public void setEnabled(long id, boolean enabled) {
		...
		if (enabled) {
			...
			DisabledInfo info = new DisabledInfo(
					"org.eclipse.pde.ui", "Disabled via PDE", desc); //$NON-NLS-1$ //$NON-NLS-2$
			PlatformAdmin platformAdmin = PDERuntimePlugin.getDefault()
					.getPlatformAdmin();
			platformAdmin.addDisabledInfo(info);
		}

		org.osgi.framework.Bundle b = PDERuntimePlugin.getDefault()
				.getBundleContext().getBundle(id);
		PackageAdmin packageAdmin = PDERuntimePlugin.getDefault()
				.getPackageAdmin();
