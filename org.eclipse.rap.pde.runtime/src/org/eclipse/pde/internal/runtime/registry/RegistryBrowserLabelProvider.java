...
public class RegistryBrowserLabelProvider extends LabelProvider {
	...
	public RegistryBrowserLabelProvider(RegistryBrowser browser) {
		...
		ImageDescriptor activePluginDesc = new OverlayIcon(
		...
		ImageDescriptor servicePropertyDesc = new OverlayIcon(
				PDERuntimePluginImages.DESC_PROPERTY_OBJ,
	...
	public Image getImage(Object element) {
		...
		if (element instanceof Property) {
			...
			if (property.getName().equals(Constants.OBJECTCLASS)) {
				return PDERuntimePluginImages
						.get(PDERuntimePluginImages.IMG_CLASS_OBJ);
			}
	...
	protected String getStyledText(Object element) {
		...
		if (element instanceof ServiceName) {
			return Arrays.asList(((ServiceName) element).getClasses())
					.toString();
		}
		...
		if (element instanceof Extension) {
			if (((RegistryBrowserContentProvider) fRegistryBrowser
					.getAdapter(IContentProvider.class)).isInExtensionSet) {
			...
			return NLS
					.bind(PDERuntimeMessages.get().RegistryBrowserLabelProvider_contributedBy,
							contributor);
		...
		if (element instanceof BundlePrerequisite) {
			...
			if (version != null) {
				if (Character.isDigit(version.charAt(0)))
					version = '(' + version + ')';
				ss.append(' ')
						.append(version/* , StyledString.DECORATIONS_STYLER */);
			}
