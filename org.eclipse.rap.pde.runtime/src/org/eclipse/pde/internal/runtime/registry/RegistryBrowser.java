...
public class RegistryBrowser extends ViewPart {
	...
	private ViewerFilter fActiveFilter = new ViewerFilter() {
		public boolean select(Viewer viewer, Object parentElement,
			...
			if (element instanceof ExtensionPoint)
				element = Platform.getBundle(((ExtensionPoint) element)
						.getNamespaceIdentifier());
			else if (element instanceof Extension)
				element = Platform.getBundle(((Extension) element)
						.getNamespaceIdentifier());
	...
	/*
	 * customized DrillDownAdapter which modifies enabled state of showing
	 * active/inactive plug-ins action - see Bug 58467
	 */
	...
	private void initializeModel() {
		...
		initializeModelJob = new Job(
				PDERuntimeMessages.get().RegistryBrowser_InitializingView) {
	...
	private void createTreeViewer(Composite parent) {
		...
		fFilteredTree = new RegistryFilteredTree(this, composite, SWT.MULTI,
				new PatternFilter());
		...
		PlatformUI
				.getWorkbench()
				.getHelpSystem()
				.setHelp(fTreeViewer.getControl(),
	...
	private void fillToolBar() {
		...
		IMenuManager manager = new MenuManager(
				PDERuntimeMessages.get().RegistryBrowser_GroupBy);
	...
	public void saveState(IMemento memento) {
		if (memento == null || fMemento == null || fTreeViewer == null)
			return;
		fMemento.putString(SHOW_RUNNING_PLUGINS,
				Boolean.toString(fShowPluginsAction.isChecked()));
		fMemento.putString(SHOW_DISABLED_MODE,
				Boolean.toString(fShowDisabledAction.isChecked()));
		fMemento.putBoolean(SHOW_ADVANCED_MODE,
				fShowAdvancedOperationsAction.isChecked());
	...
	private void makeActions() {
		...
		fRefreshAction
				.setText(PDERuntimeMessages.get().RegistryView_refresh_label);
		fRefreshAction
				.setToolTipText(PDERuntimeMessages.get().RegistryView_refresh_tooltip);
		...
		fShowPluginsAction = new Action(
				PDERuntimeMessages.get().RegistryView_showRunning_label) {
			public void run() {
				if (fShowPluginsAction.isChecked()) {
					fTreeViewer.addFilter(fActiveFilter);
				} else {
		...
		fShowPluginsAction.setChecked(fMemento.getString(SHOW_RUNNING_PLUGINS)
				.equals("true")); //$NON-NLS-1$

		fShowDisabledAction = new Action(
				PDERuntimeMessages.get().RegistryView_showDisabled_label) {
			public void run() {
				if (fShowDisabledAction.isChecked()) {
					fTreeViewer.addFilter(fDisabledFilter);
				} else {
		...
		fShowDisabledAction.setChecked(fMemento.getString(SHOW_DISABLED_MODE)
				.equals("true")); //$NON-NLS-1$
		...
		fGroupByBundlesAction = new GroupByAction(
				PDERuntimeMessages.get().RegistryBrowser_Bundle, BUNDLES);
		int groupBy = getGroupBy();
		fGroupByBundlesAction.setChecked(groupBy == BUNDLES);
		fGroupByExtensionPointsAction = new GroupByAction(
				PDERuntimeMessages.get().RegistryBrowser_ExtensionPoint,
				EXTENSION_REGISTRY);
		fGroupByExtensionPointsAction.setChecked(groupBy == EXTENSION_REGISTRY);
		fGroupByServicesAction = new GroupByAction(
				PDERuntimeMessages.get().RegistryBrowser_Service, SERVICES);
		fGroupByServicesAction.setChecked(groupBy == SERVICES);

		fShowAdvancedOperationsAction = new Action(
				PDERuntimeMessages.get().RegistryView_showAdvanced_label) {
			public void run() { // do nothing
			}
		};
		fShowAdvancedOperationsAction.setChecked(fMemento.getString(
				SHOW_ADVANCED_MODE).equals("true")); //$NON-NLS-1$

		fStartAction = new Action(
				PDERuntimeMessages.get().RegistryView_startAction_label) {
			public void run() {
				try {
		...
		fStopAction = new Action(
				PDERuntimeMessages.get().RegistryView_stopAction_label) {
			public void run() {
				try {
					List bundles = getSelectedBundles();
					for (Iterator it = bundles.iterator(); it.hasNext();) {
		...
		fEnableAction = new Action(
				PDERuntimeMessages.get().RegistryView_enableAction_label) {
			public void run() {
				List bundles = getSelectedBundles();
				for (Iterator it = bundles.iterator(); it.hasNext();) {
					Bundle bundle = (Bundle) it.next();
		...
		fDisableAction = new Action(
				PDERuntimeMessages.get().RegistryView_disableAction_label) {
			public void run() {
				List bundles = getSelectedBundles();
				for (Iterator it = bundles.iterator(); it.hasNext();) {
					Bundle bundle = (Bundle) it.next();
		...
		fDiagnoseAction = new Action(
				PDERuntimeMessages.get().RegistryView_diagnoseAction_label) {
			public void run() {
				List bundles = getSelectedBundles();
				for (Iterator it = bundles.iterator(); it.hasNext();) {
					Bundle bundle = (Bundle) it.next();
		...
		fCollapseAllAction
				.setText(PDERuntimeMessages.get().RegistryView_collapseAll_label);
		fCollapseAllAction
				.setImageDescriptor(PDERuntimePluginImages.DESC_COLLAPSE_ALL);
		fCollapseAllAction
				.setToolTipText(PDERuntimeMessages.get().RegistryView_collapseAll_tooltip);
	}
	...
	public String getTitleSummary() {
		...
		if (tree == null)
			return NLS.bind(PDERuntimeMessages.get().RegistryView_titleSummary,
					(new String[] { "0", "0", type })); //$NON-NLS-1$ //$NON-NLS-2$
		return NLS.bind(
				PDERuntimeMessages.get().RegistryView_titleSummary,
				(new String[] { Integer.toString(tree.getItemCount()),
						Integer.toString(total), type }));
	}
		...
		if (now - lastRefresh > REFRESH_DELAY) {
			...
			Runnable runnable = new Runnable() {
				public void run() {
					...
					fTreeViewer.getTree().getDisplay()
